...
export default function FarmsScreen() {
  ...
  const handleSave = () => {
    ...
    setShowModal(false);
    setFormData({ name: '', cpfCnpj: '', city: '', state: 'SP', size: '' });
  };
          ...
          {filteredFarms.map((farm) => (
            ...
            >
              ...
              <View style={styles.farmInfo}>
                ...
                <Text style={styles.farmLocation}>{farm.location || `${farm.city} - ${farm.state}`}</Text>
            ...
            <View style={[styles.modalContent, { maxHeight: '70%' }]}>
              ...
              <ScrollView style={styles.modalBody}>
                {STATES.map((state) => (
                  <TouchableOpacity
                    key={state}
                    style={[styles.stateOption, formData.state === state && styles.stateOptionActive]}
                    onPress={() => {
                      setFormData({...formData, state});
                      setShowStateModal(false);
                    }}
                  >
                    <Text style={[styles.stateOptionText, formData.state === state && styles.stateOptionTextActive]}>
                      {state}
                    </Text>
                  </TouchableOpacity>
                ))}
...
const styles = StyleSheet.create({
  ...
  farmCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.surface, borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: Colors.border },
  farmIcon: { width: 48, height: 48, borderRadius: 24, backgroundColor: Colors.primary + '15', alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  farmInfo: { flex: 1 },
  ...
  formSelect: { backgroundColor: Colors.background, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, borderWidth: 1, borderColor: Colors.border, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  stateOption: { paddingVertical: 14, paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: Colors.border },
